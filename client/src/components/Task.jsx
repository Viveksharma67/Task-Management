...
const Task = ({ props, onDelete }) => {
    ...
    useEffect(() => {
        if (props.status === 'Pending') setBadgecolor('blue')
        else if (props.status === 'Running') setBadgecolor('yellow')
        else if (props.status === 'Completed') setBadgecolor('green')
        else if (props.status === 'Failed') setBadgecolor('red')
    }, [props.status])
    ...
    return (
        <div className="border p-3 rounded-md mb-5">
            ...
            <p className="line-clamp-2 mb-1">{props.description}</p>

            <p className="text-sm text-gray-500 mb-3">
                Due Date: {props.dueDate ? new Date(props.dueDate).toLocaleDateString() : 'No due date'}
            </p>

            <div className="flex gap-5 items-center">
                <Link
                    to={`/app/show-task/${props._id}`}
